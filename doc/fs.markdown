...
use this module do `require('fs')`. Most of the methods have asynchronous and
synchronous forms. (this is partly true, will be more true in later versions).

Most of this is available in some form in ColdFusion, but this adds a little to the mix with the automatic asynchronous callbacks, etc.
...
Here is an example of the asynchronous version:
    ...
    fs.unlink('/tmp/hello', function (err) {
      ...
      console.print('successfully deleted /tmp/hello');
    });
...
following is prone to error:
    ...
    fs.rename('/tmp/hello', '/tmp/world', function (err) {
      if (!isNull(err)) throw err;
      console.print('renamed complete');
    });
    fs.stat('/tmp/world', function (err, stats) {
      if (!isNull(err)) throw err;
      console.print('stats: ' & JSON.stringify(stats));
    });
...
Synchronous stat(2). Returns a structure of file information.
...
## fs.lstatSync(path)
**NOT AVAILABLE**
Synchronous lstat(2). Returns a structure of file information.

## fs.fstatSync(fd)
**NOT AVAILABLE**
Synchronous fstat(2). Returns a structure of file information.
...
Example:
    ...
    fs.appendFile('message.txt', 'data to append', function (err) {
      if (!isNull(err)) throw err;
      console.print('The "data to append" was appended to file!');
    });
...
stat object:
    ...
    fs.watchFile('message.text', function (curr, prev) {
      console.print('the current mtime is: ' & curr.mtime);
      console.print('the previous mtime was: ' & prev.mtime);
    });
...
callback, and have some fallback logic if it is null.
    ...
    fs.watch('somedir', function (event, filename) {
      console.print('event is: ' & event);
      if (filename) {
        console.print('filename provided: ' & filename);
      } else {
        console.print('filename not provided');
      }
    });
